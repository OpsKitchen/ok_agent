...
func (item *File) processLink() error {
	...
	if item.pathExist == true {
		if linkTarget, _ := os.Readlink(item.FilePath); linkTarget == item.Target {
			util.Logger.Info("Skip to create symbol link, because it already exists with correct target .")
			return nil
		}
		...
		if err == nil {
			...
			if convertedOk {
				...
				if item.gid == stat_t.Gid && item.uid == stat_t.Uid {
					util.Logger.Info("Skip to change ownership, because file ownership is correct.")
					return nil
				}
			}
		}
...
func (item *File) changePermission() error {
	if item.Permission != "" {
		...
		if stat.Mode().Perm() == item.perm {
			util.Logger.Info("Skip to change permission, because file permission is correct.")
			return nil
		}
...
func (item *File) createParentDir() error {
	...
	if err == nil { //path exist
		...
		util.Logger.Info("Skip to create parent directory, because it already exists.")
		return nil
	}
...
func (item *File) writeContent() error {
	if contentBytes, _ := ioutil.ReadFile(item.FilePath); item.FileContent == string(contentBytes) {
		util.Logger.Info("Skip to write content, because it is correct.")
		return nil
	}
